    ...
    public void FillNoticeDetails()
    {
        try
        {
           // strQry = "usp_StaffDashboard @type='NoticeBoard',@SchoolId='" + Session["School_id"] + "'";
            strQry = "usp_TeacherDashboard @type='NoticeBoardNew',@SchoolId='" + Session["School_id"] + "',@intUserType_id='" + Convert.ToString(Session["UserType_id"]) + "',@intUser_id='" + Convert.ToString(Session["User_id"]) + "'";
            dsObj = sGetDataset(strQry);
            if (dsObj.Tables[0].Rows.Count > 0)
            {
